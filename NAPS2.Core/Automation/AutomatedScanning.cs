...
namespace NAPS2.Automation
{
    public class AutomatedScanning
    {
        ...
        public AutomatedScanning(AutomatedScanningOptions options, IProfileManager profileManager,
            IScanPerformer scanPerformer, IErrorOutput errorOutput, IEmailProviderFactory emailProviderFactory,
            IScannedImageImporter scannedImageImporter, IUserConfigManager userConfigManager,
            PdfSettingsContainer pdfSettingsContainer, FileNamePlaceholders fileNamePlaceholders,
            ImageSettingsContainer imageSettingsContainer, IOperationFactory operationFactory,
            AppConfigManager appConfigManager, OcrManager ocrManager, IFormFactory formFactory,
        ...
        {
            var canUseOcr = IsPdfFile(options.OutputPath) || IsPdfFile(options.EmailFileName);
            var useOcr = canUseOcr && !options.DisableOcr && (options.EnableOcr || options.OcrLang != null ||
                                                              userConfigManager.Config.EnableOcr ||
                                                              appConfigManager.Config.OcrState == OcrState.Enabled);
            var ocrLanguageCode = useOcr ? (options.OcrLang ?? ocrManager.DefaultParams?.LanguageCode) : null;
        ...
        private void ReorderScannedImages()
        {
            var sep = options.SplitPatchT ? SaveSeparator.PatchT
                ...
                : SaveSeparator.None;
            scanList = SaveSeparatorHelper.SeparateScans(scanList, sep, options.SplitSize).Where(x => x.Count > 0)
                .ToList();
        ...
        private bool PreCheckOverwriteFile()
        {
            ...
            if (!IsPdfFile(subPath) || !File.Exists(subPath) || options.ForceOverwrite) return true;
        ...
        private async Task ImportImages()
        {
            ...
            foreach (var filePath in filePaths)
            {
                ...
                try
                {
                    ...
                    {
                        Slice = Slice.Parse(filePath, out var actualPath),
                        DetectPatchCodes = options.SplitPatchT,
                        NoThumbnails = true
                    };
                    var images = await scannedImageImporter
        ...
        private async Task EmailScannedImages()
        {
            ...
            try
            {
                var targetPath = Path.Combine(tempFolder.FullName, options.EmailFileName);
                if (IsPdfFile(targetPath))
                {
                    if (options.OutputPath != null && IsPdfFile(options.OutputPath))
                    {
                        ...
                        var digits = (int)Math.Floor(Math.Log10(scanList.Count)) + 1;
                        var i = 0;
                        foreach (var path in actualOutputPaths)
                        {
                            var attachmentName = fileNamePlaceholders.SubstitutePlaceholders(options.EmailFileName,
                                startTime, false, i++, scanList.Count > 1 ? digits : 0);
        ...
        public bool ValidateOptions()
        {
            switch (options.OutputPath)
            {
                ...
                case null when options.EmailFileName == null && options.Install == null && !options.AutoSave:
                    errorOutput.DisplayError(ConsoleResources.OutputOrEmailRequired);
                    return false;
                case null when options.EmailFileName == null && options.ImportPath != null:
        ...
        private async Task DoExportToImageFiles(string outputPath)
        {
            ...
            imageSettingsContainer.ImageSettings = new ImageSettings
            {
                ...
                TiffCompression = Enum.TryParse<TiffCompression>(options.TiffComp, true, out var tc)
                    ? tc
            ...
            foreach (var scan in scanList)
            {
                ...
                {
                    if (op.Status.CurrentProgress <= i) return;
        ...
        private async Task<bool> DoExportToPdf(string path, bool email)
        {
            ...
            var encryption = options.UseSavedEncryptConfig
                ? pdfSettingsContainer.PdfSettings.Encryption
                : new PdfEncryption();
            ...
            foreach (var fileContents in scanList)
            {
                ...
                var digits = (int)Math.Floor(Math.Log10(scanList.Count)) + 1;
                var actualPath = fileNamePlaceholders.SubstitutePlaceholders(path, startTime, true, scanIndex++,
                    scanList.Count > 1 ? digits : 0);
            ...
            try
            {
                ...
                else
                {
                    ...
                    profile = profileManager.Profiles.FirstOrDefault(x => x.DisplayName == options.ProfileName) ??
                              profileManager.Profiles.First(x =>
                                  x.DisplayName.ToLower() == options.ProfileName.ToLower());
                }
            }

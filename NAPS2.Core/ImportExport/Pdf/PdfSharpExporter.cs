...
namespace NAPS2.ImportExport.Pdf
{
    public class PdfSharpExporter : IPdfExporter
    {
        ...
        private bool PageContainsText(PdfPage page)
        {
            ...
            {
                var textAndFormatting = elements.GetDictionary(i).Stream.ToString();
        ...
        private static PdfPage CopyPdfPageToDoc(PdfDocument destDoc, ScannedImage image)
        {
            // Pull the PDF content directly to maintain objects, dpi, etc.
            var sourceDoc = PdfReader.Open(image.RecoveryFilePath, PdfDocumentOpenMode.Import);
            var sourcePage = sourceDoc.Pages.Cast<PdfPage>().Single();
            var destPage = destDoc.AddPage(sourcePage);
        ...
        {
            var hAdjust = 72 / img.HorizontalResolution;
            var vAdjust = 72 / img.VerticalResolution;
            if (double.IsInfinity(hAdjust) || double.IsInfinity(vAdjust))
            {
                hAdjust = vAdjust = 0.75;
            }
            var realWidth = img.PixelWidth * hAdjust;
            var realHeight = img.PixelHeight * vAdjust;
        ...
        {
            var fontSizeGuess = Math.Max(1, (int)(adjustedBounds.Height));
            var measuredBoundsForGuess = gfx.MeasureString(text, new XFont("Times New Roman", fontSizeGuess, XFontStyle.Regular));
            var adjustmentFactor = adjustedBounds.Width / measuredBoundsForGuess.Width;
            var adjustedFontSize = Math.Max(1, (int)Math.Floor(fontSizeGuess * adjustmentFactor));

    ...
    public class PdfSharpImporter : IPdfImporter
    {
        ...
        public ScannedImageSource Import(string filePath, ImportParams importParams, ProgressHandler progressCallback, CancellationToken cancelToken)
        {
            ...
            Task.Factory.StartNew(async () =>
            {
                ...
                try
                {
                    var document = PdfReader.Open(filePath, PdfDocumentOpenMode.Import, args =>
                    {
                        if (pdfPasswordProvider.ProvidePassword(Path.GetFileName(filePath), passwordAttempts++,
                            out args.Password)) return;
                        args.Abort = true;
                        aborted = true;
                    });
        ...
        private async Task GetImagesFromPage(PdfPage page, ImportParams importParams, ScannedImageSource.Concrete source)
        {
            ...
            foreach (var item in xObjects.Elements.Values)
            {
                ...
                if (!((item as PdfReference)?.Value is PdfDictionary xObject) ||
                    xObject.Elements.GetString("/Subtype") != "/Image") continue;
        ...
        {
            var pdfPath = Path.Combine(Paths.Temp, Path.GetRandomFileName());
            var document = new PdfDocument();
            document.Pages.Add(page);
            document.Save(pdfPath);

            var image = ScannedImage.FromSinglePagePdf(pdfPath, false);
            if (importParams.NoThumbnails && !importParams.DetectPatchCodes) return image;
        ...
        {
            var width = imageObject.Elements.GetInteger(PdfImage.Keys.Width);
            var height = imageObject.Elements.GetInteger(PdfImage.Keys.Height);
            var bitsPerComponent = imageObject.Elements.GetInteger(PdfImage.Keys.BitsPerComponent);
        ...
        private static void RgbToBitmapUnmanaged(int height, int width, Bitmap bitmap, byte[] rgbBuffer)
        {
            ...
            try
            {
                for (var y = 0; y < height; y++)
                {
                    ...
                    {
                        var pixelData = data.Scan0 + y * data.Stride + x * 3;
                        var bufferIndex = (y * width + x) * 3;
        ...
        private static void BlackAndWhiteToBitmapUnmanaged(int height, int width, Bitmap bitmap, byte[] bwBuffer)
        {
            ...
            try
            {
                var bytesPerRow = (width - 1) / 8 + 1;
                for (var y = 0; y < height; y++)
                {
                    for (var x = 0; x < bytesPerRow; x++)
                    {
                        var pixelData = data.Scan0 + y * data.Stride + x;

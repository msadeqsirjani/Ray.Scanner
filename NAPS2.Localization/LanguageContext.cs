...
namespace NAPS2.Localization
{
    public class LanguageContext
    {
        ...
        private void TranslateFile(FileSystemInfo file, bool winForms)
        {
            var doc = XDocument.Load(file.FullName);
            var hasSomethingTranslatable = false;

            foreach (var item in doc.Root.Elements("data").ToList())
            {
                var prop = item.Attribute("name")?.Value;
                var original = item.Element("value")?.Value;
                if (!Rules.IsTranslatable(winForms, prop, ref original, out var prefix, out var suffix))
                {
                ...
                if (!Strings.ContainsKey(original)) continue;
            ...
            var savePath = file.FullName.Replace(".resx", $".{langCode}.resx");
            doc.Save(savePath);

            UpdateProjectFile(file.FullName, savePath);
        }

        private static void UpdateProjectFile(string dependentPath, string savePath)
        {
            var relativePath = RelativePath(savePath);
            var relativeDependentPath = RelativePath(dependentPath);
            var projectFilePath = Path.Combine(Paths.Root, @"NAPS2.Core\NAPS2.Core.csproj");
            var doc = XDocument.Load(projectFilePath);
            const string schema = "http://schemas.microsoft.com/developer/msbuild/2003";

            var oldElement = doc.Descendants(XName.Get("EmbeddedResource", schema))
                .FirstOrDefault(x => x.Attribute("Include")?.Value == relativePath);
            if (oldElement != null)
            {
            ...
            var dependentElement = doc.Descendants(XName.Get("EmbeddedResource", schema))
                .FirstOrDefault(x => x.Attribute("Include")?.Value == relativeDependentPath);
            ...
            var siblings = doc.Descendants(XName.Get("EmbeddedResource", schema))
                .Where(x => SameDependentFile(x.Attribute("Include")?.Value, relativePath))
                .OrderBy(x => x.Attribute("Include")?.Value).ToList();
            int index;
            ...
            if (siblings.Count > 0)
            {
                ...
                newElement.Add(
                    new XElement(XName.Get("DependentUpon", schema), Path.GetFileName(relativeDependentPath)));
                if (index == 0)
                {
